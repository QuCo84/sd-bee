...
function HTML_stripTags($html, $keepTags = array())
{
  $p1 = $p2 = 0;
  $safe = 10000;
  //if ( strlen($html) > 1000) { var_dump( $html); die();}
  while (($p1 = strpos($html, '<', $p2))> -1 && $safe--)
  {
    $p2 = strpos($html, '>', $p1);
   ...
   for ($i=0; $i<$nbQueryTokens;$i++)
   {
      ...
      switch ($queryToken[0])
      {
          ...
          case ':' :
            ...
            break;
          case '@' :
            // Search for itemprop="token  
            $queryToken = str_replace('_SP_', ' ', $queryToken);
            $startRegex .= ' itemprop="'.substr( $queryToken, 1);
            $capture = '[^<]*';
            break;
          case '=' :
          ...
          case '^' : 
	       ...
	       $seq = "";
               while ( $skipi--) $seq = HTML_addQueryStep( $skipTags[ $skipi], $seq); // .= if more than 1
               $startRegex .= $seq;
            }
            else { // old format
          ...
          default :
	       ...
	       if ( $startRegex) $startRegex .= '[^>]*>[^<]*';
               ...
               $capture = "[A-Za-zÀ-ÿ0-9\h,'][A-Za-zÀ-ÿ0-9\h,'][A-Za-zÀ-ÿ0-9\h,'][^<]*";     //           
            } else {
               if ( $startRegex) $startRegex .= '[^>]*>[^<]*';
               $startRegex .= "<{$queryToken}";
...
function HTML_extractHashtags( &$html) {
   ...
   $hashtags = $matches[0];
   //for ( $matchi=0; $matchi < count( $matches); $matchi++) $hashtags[] = 
